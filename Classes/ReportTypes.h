typedef enum { 
	ReportTypeDay = 0, 
	ReportTypeWeek = 1, 
	ReportTypeFinancial = 2, 
	ReportTypeFree = 3, 
	ReportTypeUnknown = 99 } ReportType;

typedef enum { 
	ReportRegionUnknown = 0, 
	ReportRegionUSA = 1, 
	ReportRegionEurope = 2, 
	ReportRegionCanada = 3, 
	ReportRegionAustralia = 4, 
	ReportRegionUK = 5, 
	ReportRegionJapan = 6, 
	ReportRegionRestOfWorld = 7,
	ReportRegionMexico = 8,
	ReportRegionSwitzerland = 9,
	ReportRegionNorway = 10,
	ReportRegionNewZealand = 11,
    ReportRegionChina = 12,
    ReportRegionDenmark = 13,
    ReportRegionSweden = 14,
    ReportRegionSingapore = 15,
    ReportRegionHongKong = 16,
    ReportRegionTaiwan = 17,
    ReportRegionRussia = 18,
    ReportRegionTurkey =19,
    ReportRegionIndia = 20,
    ReportRegionIndonesia = 21,
    ReportRegionIsrael = 22,
    ReportRegionSaudiArabia = 23,
    ReportRegionSouthAfrica = 24,
    ReportRegionUnitedArabEmirates = 25} ReportRegion;
